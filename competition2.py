...
def plot_total_mass(tgrid: np.ndarray, ygrid: np.ndarray) -> None:
    ...
    plt.plot(tgrid, P1tot * 1e-4, color='green' , label=r"$u_1$")
    plt.plot(tgrid, P2tot * 1e-4, linestyle="-.", color='orange', label=r"$u_2$")
    plt.legend()
    plt.title("Total mass of $u_1$, $u_2$ over time")
    plt.xlabel(r"$t$")
    plt.ylabel(r"$\int_\Omega u_i\,dx$")
    plt.tight_layout()
...
def plot_fields_at_times(tgrid: np.ndarray, ygrid: np.ndarray, times_to_plot: list[int] | np.ndarray) -> None:
    ...
    axes[2, 0].set_ylabel("Substrate $c$")
